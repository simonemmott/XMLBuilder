...
```
Which uses two space characters `'  '` as the indent. Any other string can be used as the indent.
...
**Note** All the `attribute(...)` methods are available on all the elements in the document including the root element.
...
only those nodes that are either the node on which the `toXml(...)` method was called or its descendant nodes.
...
.toXml(myFile).close();
```
**Note** the generated xml is automatically flushed to the file, but the file is not automatically closed.
